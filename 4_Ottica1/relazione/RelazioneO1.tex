...
Si nota subito che il $\chi^2$ sia completamente fuori scala. Questo è anche ben visibile dal plot dei residui normalizzati \ref{fig:Idrogeno, Residui normalizzati}.
...
A questo punto si è sostituita all lampada ad idrogeno la lampada al sodio. Ci si è focalizzati sul doppietto giallo (tra l'altro l'unico doppietto visibile realmente attribuibile al sodio). Nella tabella sotto \ref{tab:NaR} gli angoli e le lunghezze ottenute.
%SCRIVO QUA COSA CHIEDE LA SCHEDA COSI DA VERIFICARE CHE CI SIA TUTTO E NON FARE AVENTI E INDIETRO CON LA PAGINA: Si utilizzi ora la lampada al sodio e si osservi la posizione angolare, al primo ordine di diffrazione, delle righe di emissione del doppietto del sodio. La distanza tra le due righe è nota essere 0.6 nm;OK SI UTILIZZI questo dato per stimare la precisione sulla misura di lunghezze d’onda ottenibile con lo spettroscopio a disposizione e SI CONFRONTI il valore ottenuto con gli errori stimati sulla lunghezza d’onda propagando l’errore sulle misure angolari al punto a).OK INFINE SI COFRONTI la misura del doppietto del sodio osservato con lo spettrometro a reticolo e con quello a prisma. Si commentino le differenze osservate. OK
...
La cosa più curiosa è che le stesse linee sono state osservate nella lampada al sodio usata per la parte uno (linee presenti nella tabella \ref{tab:moreNa}s).
...
Sarebbe forse plausibile che le linee appartengano agli atomi degli elettrodi o all'idrogeno molecolare. 
